...
function mySnakeGame_final(command)
...
switch command
    ...
    % Change game mode
    case 'Mode' 
        set(get(get(gcbo,'Parent'),'Children'),'checked','off') % Uncheck the child menu items
        set(gcbo,'checked','on') % Check the selected menu item

        % Configure game mode based on the selected label
        switch get(gcbo,'Label')
            case 'Original'
                mode = getBackground();
            case 'Box' % Configure barriers for the "Box" mode
                mode([1 40],:) = 1;
                mode(:,[1 40]) = 1;
            case 'Maze' % Configure barriers for the "Maze" mode
                mode(1:5,5) = 1;
                mode(1:25,25) = 1;
                mode(10,1:5) = 1;
                mode(5,35:40) = 1;
        ...
        feval(mfilename,'Stop') % Stop the game when the mode changes

    % Change the level
    case 'Level' 
        set(get(get(gcbo,'Parent'),'Children'),'checked','off') % Uncheck the child menu items
        set(gcbo,'checked','on') % Check the selected menu item
        level = str2double(get(gcbo,'Label')); % convert values of level from string to double
